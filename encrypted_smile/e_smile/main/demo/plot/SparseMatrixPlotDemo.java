...
import data.parser.SparseMatrixParser;
import math.matrix.SparseMatrix;
import plot.Palette;
import plot.PlotCanvas;
import plot.SparseMatrixPlot;
...
public class SparseMatrixPlotDemo extends JPanel {
    public SparseMatrixPlotDemo() {
        ...
        try {
            SparseMatrix m1 = parser.parse(data.parser.IOUtils.getTestDataFile("matrix/08blocks.txt"));
            PlotCanvas canvas = SparseMatrixPlot.plot(m1);
            canvas.setTitle("08blocks");
            add(canvas);

            SparseMatrix m2 = parser.parse(data.parser.IOUtils.getTestDataFile("matrix/mesh2em5.txt"));
